 ...
 * reported, discovered while wiring `#4094`'s `.select()`/`selector` MCP
 * tool parameter, which can legitimately translate `Qto_WallBaseQuantities.
 * NetVolume>1` into this same `QueryFilter` shape). Mirrors the CLI
 * `--where` flag's own fallback (`applyWhereFilter` in
 * `packages/cli/src/commands/where-filter.ts`) so `bim.query().where('Qto_...',
 * ...)` agrees across CLI and MCP again.
 */
