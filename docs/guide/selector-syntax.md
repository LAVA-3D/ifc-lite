...
| `GlobalId=`, `GlobalId!=` | ✅ | reuses the same globalId rule the bare term builds; `*=`, `>`/`>=`/`<`/`<=` and `/regex/` are reported |
...
than text to search within or order, so `*=`, `>`/`>=`/`<`/`<=` and a
`/regex/` value stay reported; use `=` or `!=` against a literal id, or the
